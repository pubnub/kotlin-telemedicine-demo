...
/** Copy of Coil class to reuse it */
/** The base [Decoder] that uses [BitmapFactory] to decode a given [BufferedSource]. */
open class BitmapFactoryDecoder(private val context: Context) : Decoder {
    ...
    private fun decodeInterruptible(
        ...
        if (shouldReadExifData(outMimeType)) {
            val exifInterface =
                ExifInterface(ExifInterfaceInputStream(safeBufferedSource.peek().inputStream()))
        ...
        when {
            ...
            size !is PixelSize -> {
                ...
                inSampleSize = DecodeUtils.calculateInSampleSize(srcWidth,
                    srcHeight,
                    width,
                    height,
                    options.scale)
        ...
        val bitmap =
            applyExifTransformations(pool, outBitmap, inPreferredConfig, isFlipped, rotationDegrees)
